...
# VISUALS-MOD: the original version guard mis-parses on torch>=2 (Python evaluates
# `False < 9` as True), importing the long-removed _LinearWithBias. Compare the
# (major, minor) tuple properly instead.
...
# VISUALS-MOD: same version-guard fix as above (was buggy for torch>=2).
if _tv < (1, 7):
