...
class SetCriterion(nn.Module):
    ...
    def loss_angles(self, outputs, targets, indices, num_boxes):  
        ...
        # VISUALS-MOD: device-agnostic (was hardcoded .cuda()).
        cls_onehot = torch.zeros(heading_target_cls.shape[0], 12, device=heading_input.device).scatter_(dim=1, index=heading_target_cls.view(-1, 1), value=1)
    ...
    def loss_depth_map(self, outputs, targets, indices, num_boxes):
        ...
        # VISUALS-MOD: derive the depth-map grid (W_d, H_d) and device from the
        # actual logits instead of hardcoding KITTI's [80, 24] / 'cuda'.
        _, _, h_d, w_d = depth_map_logits.shape
        dev = depth_map_logits.device
        gt_boxes2d = torch.cat([t['boxes'] for t in targets], dim=0) * torch.tensor([w_d, h_d, w_d, h_d], device=dev)
...
def build(cfg):
    ...
    # VISUALS-MOD: expose group_num (Group-DETR groups) via cfg, default 11 as
    # upstream. group_num=1 makes training and inference use the same query set
    # (no train/infer mismatch) — preferred for from-scratch training.
    ...
    criterion = SetCriterion(
        ...
        losses=losses,
        group_num=group_num)  # VISUALS-MOD: keep criterion groups in sync with model
