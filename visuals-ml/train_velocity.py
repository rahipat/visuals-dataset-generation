...
def run_epoch(model, loader, criterion, optimizer, scaler, device, train: bool, epoch: int, total_epochs: int):
    ...
    with torch.set_grad_enabled(train):
        for batch_idx, (crop_t, crop_t1, coords, target) in enumerate(loader):
...
def main():
    ...
    for epoch in range(start_epoch, cfg["epochs"] + 1):
        ...
        print(f"Epoch {epoch:3d}/{cfg['epochs']}  train={train_loss:.6f}  val={val_loss:.6f}", flush=True)

        if val_loss < best_val_loss:
            best_val_loss = val_loss
            torch.save({"epoch": epoch, "model": model.state_dict(), "val_loss": val_loss},
                       checkpoint_dir / "velocity_best.pt")
            print(f"  --> checkpoint saved (val_loss={val_loss:.6f})", flush=True)
